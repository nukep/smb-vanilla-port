...
void set_error_context(const char *context) { ERROR_CONTEXT = context; }

#define IOOPEN_READONLY(filename)       \
  FILE *__file = fopen(filename, "rb"); \
  if (!__file) {                        \
    error_cb(strerror(errno));          \
    goto err;                           \
  }
#define IOOPEN_WRITEONLY(filename)      \
  FILE *__file = fopen(filename, "wb"); \
  if (!__file) {                        \
    error_cb(strerror(errno));          \
    goto err;                           \
  }
#define IOSEEKTO(offset)                          \
  if (fseek(__file, offset, SEEK_SET) != 0) {     \
    error_cb("Could not seek to offset in file"); \
    goto err;                                     \
  }
#define IOREAD(size, buf)                     \
  if (fread(buf, size, 1, __file) != 1) {     \
    error_cb("Could not read bytes in file"); \
    goto err;                                 \
  }
#define IOWRITE(size, buf)                     \
  if (fwrite(buf, size, 1, __file) != 1) {     \
    error_cb("Could not write bytes in file"); \
    goto err;                                  \
  }
#define IOCLOSE()                     \
  if (fclose(__file) != 0) {          \
    error_cb("Could not close file"); \
    goto err;                         \
  }                                   \
  return true;                        \
err:                                  \
...
void joy1(void *userdata, struct SMB_buttons *buttons) { *buttons = PLAYER1_INPUTS; }
  ...
  while (SDL_PollEvent(&eventData)) {
    ...
    case SDL_KEYUP: {
      ...
      if (!use_movie_buttons) {
        switch (sc) {
        case SDL_SCANCODE_W:
          PLAYER1_INPUTS.u = isdown;
          break;
        case SDL_SCANCODE_S:
          PLAYER1_INPUTS.d = isdown;
          break;
        case SDL_SCANCODE_A:
          PLAYER1_INPUTS.l = isdown;
          break;
        case SDL_SCANCODE_D:
          PLAYER1_INPUTS.r = isdown;
          break;
        case SDL_SCANCODE_J:
          PLAYER1_INPUTS.b = isdown;
          break;
        case SDL_SCANCODE_K:
          PLAYER1_INPUTS.a = isdown;
          break;
        case SDL_SCANCODE_U:
          PLAYER1_INPUTS.select = isdown;
          break;
        case SDL_SCANCODE_I:
          PLAYER1_INPUTS.start = isdown;
          break;
        }
      }
    } break;
...
int sdl_frontend(void *userdata) {
  ...
  window = SDL_CreateWindow("smb-port", SDL_WINDOWPOS_UNDEFINED, SDL_WINDOWPOS_UNDEFINED, 256 * fe->video_scale,
                            240 * fe->video_scale, SDL_WINDOW_SHOWN);
  if (window == NULL) {
