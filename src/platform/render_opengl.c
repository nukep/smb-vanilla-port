...
// Uses API generated by GLAD 2. Avoids platform-specific libraries such as SDL, by design.
...
bool SMBgl_init(struct SMBgl *gl, SMBgl_GetProcAddress_f get_proc_address) {
  ...
  int version = gladLoadGL(get_proc_address);
  if (version == 0) {
    log_error("Could not run gladLoadGL");
    goto error;
  }

  log_info("OpenGL version %d.%d", GLAD_VERSION_MAJOR(version), GLAD_VERSION_MINOR(version));
...
void SMBgl_fini(struct SMBgl *gl) {
  // If any of these are uninitialized (= 0), it's still safe to free them
